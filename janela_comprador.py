...
class Janela_comprador(Toplevel):
    def __init__(self, parent, control):
        ...
        self.bt_remove = Button(self, text='Remover Comprador', command=self.remove_comp). \
        ...
        self.entry_cpf = Entry(self, textvariable=self.entry_cpf_var2). \
            grid(row=2, column=3, padx=1, pady=1)
        self.lbl_cpf = Label(self, text='CPF'). \
            grid(row=2, column=1, padx=1, pady=1)
        ...
        for comp in self.control.bd.show_comp():
            if cpf == comp.get_cpf():
                if messagebox.askyesno\
                            ('Excluir', f'Tem certeza que deseja excluir o comprador {comp.get_nome()}?') is False:
                    return None
